...
export class MeetingListProviderService {
  ...
  meetings: any;
  nernaBMLT = environment.nernaBMLT;
  tomatoBMLT = environment.tomatoBMLT;
  tomatoNernaBMLT = environment.tomatoNernaBMLT;

  constructor(public http: HttpClient) {
    console.log('Hello MeetingListProvider Provider');
  }

  getApiUrlMap: string = this.nernaBMLT + '?switcher=GetSearchResults&sort_keys=longitude,latitude,weekday_tinyint,start_time';
  getApiUrlDay: string = this.nernaBMLT + '?switcher=GetSearchResults&sort_keys=weekday_tinyint,start_time';
  ...
  getCircleMeetings(lat: string, long: string, radius: string) {
    const getApiUrlCircleMap: string = this.tomatoBMLT
      ...
      + '&long_val=' + long
      + '&lat_val=' + lat
      + '&sort_keys=longitude,latitude&callingApp=na-nerna-ionic';
  ...
  getMeetingsByAreaProvider(areaID: string) {
    const getMeetingsByAreaURL: string = this.nernaBMLT
      + '?switcher=GetSearchResults&services='
      + areaID
      + '&sort_keys=weekday_tinyint,start_time&callingApp=na-nerna-ionic';
    return this.http.get(getMeetingsByAreaURL);
  }

  getCounties() {
    const getCountiesURL: string = this.nernaBMLT
      + '?switcher=GetSearchResults&services=1&recursive=1&data_field_key=location_sub_province&sort_keys=location_sub_province';
    return this.http.get(getCountiesURL);
  }

  getMeetingsByCountyProvider(countyName: string) {
    const getCountyMeetingsURL: string = this.nernaBMLT
      + '?switcher=GetSearchResults&services=1&recursive=1&data_field_key=location_sub_province&sort_keys=location_sub_province';
  ...
  getNernaTomatoMeetings() {
    return this.http.get(this.tomatoNernaBMLT);
  }
}
