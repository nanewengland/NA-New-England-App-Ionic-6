...
export class FormatsService {
  ...
  nernaBMLT = 'https://nerna.org/main_server/client_interface/json/?switcher=GetFormats&show_all=1&format_ids=';

  constructor(private httpCors: HTTP) {}

  async getFormatNamesByID(uniqueIDs: Set<string>, language) {
    const formatNamesByID = {};
    const formatsApi = this.nernaBMLT + Array.from(uniqueIDs).join(",");
  ...
  setExplodedFormatsOnMeetingList(meetingList, formatLanguage) {
    ...
    for (const meeting of meetingList) {
      for (const formatID of meeting?.formats.split(",") || []) {
